...
import CapitalMap from '@/app/dashboard/assessment/components/CapitalMap';
...
import { gradeR7ZoneForm, UNIQUE_R7_DEVELOPERS, UNIQUE_R7_PROJECTS, CAPITAL_R7_ZONE_ANSWERS } from '@/lib/assessment/data/zone-answers-capital-r7';
import CapitalIntroModal from '@/app/dashboard/assessment/components/CapitalIntroModal';

const TOTAL = CAPITAL_R7_ZONES.filter(z => !CAPITAL_R7_NO_PIN_IDS.has(z.id)).length;
...
export default function CapitalGameR7Page() {
  ...
  useEffect(() => {
    ...
    if (!sessionId || isCapital !== '2') { router.replace('/dashboard/assessment'); return; }
    if (!localStorage.getItem('va_capital_r7_intro_seen')) {
  ...
  const unansweredZones = CAPITAL_R7_ZONES.filter(z => !answeredIds.has(z.id) && !CAPITAL_R7_NO_PIN_IDS.has(z.id));
  const activeZone      = activeZoneId ? CAPITAL_R7_ZONES.find(z => z.id === activeZoneId) : null;
  ...
  const handleStarClick = useCallback((zoneId: string) => {
    ...
    if (zoneResults[zoneId] !== undefined) return;
  ...
  const handleZoneSubmit = useCallback(async () => {
    ...
    setSaving(true);
    const sessionId = localStorage.getItem('va_session_id');
    const { allOk } = gradeR7ZoneForm(formState, activeZoneId);
    if (sessionId) {
      await saveAnswers(sessionId, [{
        phase: 'capital_r7_map',
  ...
  const handleNext = useCallback(async () => {
    ...
    router.push('/dashboard/assessment/capital-quiz-r7');
  }, [unansweredZones, router]);

  const handleCloseIntro = useCallback(() => {
    if (!introForceOpen) {
      localStorage.setItem('va_capital_r7_intro_seen', '1');
    }
    setShowIntro(false);
    ...
    >
      ...
      <div className="flex-1 overflow-hidden p-3" style={{ position: 'relative', minHeight: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ opacity: mapReady ? 1 : 0, transition: 'opacity 300ms', width: '100%' }}>
          <CapitalMap
            zones={CAPITAL_R7_ZONES}
            zoneResults={zoneResults}
            part2Results={part2Results}
            onStarClick={handleStarClick}
            onReady={() => setMapReady(true)}
            mapSrc="/r7-capital-map.png"
            mapW={1998}
            mapH={1563}
            zoneAnswers={CAPITAL_R7_ZONE_ANSWERS}
            noPinIds={CAPITAL_R7_NO_PIN_IDS}
          />
        </div>
      </div>
      ...
      {/* ── Part 2 Modal ── */}
      {part2ActiveZoneId && (() => {
        const zone = CAPITAL_R7_ZONES.find(z => z.id === part2ActiveZoneId);
        if (!zone) return null;
        return (
          <Part2Modal
            zoneCode={zone.code}
