...
export default function CapitalQuizPage() {
  ...
  const [questions] = useState(() => shuffle(CAPITAL_QUESTIONS));
  ...
  async function finish(submittedAnswers: (string | null)[]) {
    ...
    if (sessionId) {
      await saveAnswers(sessionId, questions.map((q, i) => {
        ...
        return { phase: 'capital_quiz', question_id: q.id, answer_given: given, correct };
      }));
  ...
  if (!question) {
    // No questions yet — skip directly to results
    finish(answers);
    return null;
  }
