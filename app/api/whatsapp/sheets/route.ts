...
export async function POST(request: NextRequest) {
  ...
  // Scope the sheet to only the uploading agent — this is what makes it
  // private (refill's scoping check only ever lets this agent claim from it).
  const { error: scopeError } = await adminClient
    .from('whatsapp_sheet_agents')
    .insert({ sheet_id: sheet.id, agent_id: userId, company_id: companyId })
